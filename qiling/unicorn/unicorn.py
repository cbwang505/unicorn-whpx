...
_setup_prototype(_uc, "uc_map_dump_memory", ucerr, uc_engine)
_setup_prototype(_uc, "uc_free_dump_memory", ucerr, uc_engine)
...
_setup_prototype(_uc, "uc_mem_map_guest", ucerr, uc_engine, ctypes.POINTER(ctypes.c_uint64), ctypes.c_size_t, ctypes.c_uint32)
_setup_prototype(_uc, "uc_mem_map_ptr", ucerr, uc_engine, ctypes.c_uint64, ctypes.c_size_t, ctypes.c_uint32, ctypes.c_void_p)
_setup_prototype(_uc, "uc_mem_map_ptr_guest", ucerr, uc_engine, ctypes.POINTER(ctypes.c_uint64), ctypes.c_size_t, ctypes.c_uint32, ctypes.c_void_p)
...
_setup_prototype(_uc, "uc_open_dump_file",ucerr, uc_engine, ctypes.c_char_p)
_setup_prototype(_uc, "uc_map_dump_section",ucerr, uc_engine, ctypes.c_char_p)
_setup_prototype(_uc, "uc_load_dump_image",ucerr, uc_engine, ctypes.c_char_p,ctypes.POINTER(ctypes.c_uint64), ctypes.POINTER(ctypes.c_size_t))
...
def reg_read(reg_read_func, arch, reg_id, opt=None):
    if arch == uc.UC_ARCH_X86 or arch == uc.UC_ARCH_X86_WHPX:
...
class Uc(object):
    ...
    def emu_stop(self) -> None:
        ...
        if status != uc.UC_ERR_OK:
            raise UcError(status)
    # map_dump_memory
    def map_dump_memory(self) -> None:
        status = _uc.uc_map_dump_memory(self._uch)
        if status != uc.UC_ERR_OK:
            raise UcError(status)
    # free_dump_memory
    def free_dump_memory(self) -> None:
        status = _uc.uc_free_dump_memory(self._uch)
        if status != uc.UC_ERR_OK:
            raise UcError(status)  
    ...
    def mem_map(self, address: int, size: int, perms: int=uc.UC_PROT_ALL):
        ...
        if status != uc.UC_ERR_OK:
            raise UcError(status)
    # map a range of memory
    def mem_map_guest(self, address: int, size: int, perms: int=uc.UC_PROT_ALL):
        regaddress = ctypes.c_uint64(address)       
        status = _uc.uc_mem_map_guest(self._uch, ctypes.byref(regaddress),  size, perms)
        if status != uc.UC_ERR_OK:
            raise UcError(status)
        return  regaddress.value 
        
    # map a range of memory
    def load_dump_image(self, file:str,address: int, size: int):
        regaddress = ctypes.c_uint64(address)       
        regsize = ctypes.c_uint64(size)       
        status = _uc.uc_load_dump_image(self._uch,ctypes.c_char_p(file.encode('utf-8')), ctypes.byref(regaddress), ctypes.byref(regsize))
        if status != uc.UC_ERR_OK:
            raise UcError(status)
        return  regaddress.value ,regsize.value
    # map a range of memory
    def open_dump_file(self, file:str):           
        status = _uc.uc_open_dump_file(self._uch,ctypes.c_char_p(file.encode('utf-8')))
        if status != uc.UC_ERR_OK:
            raise UcError(status)
    def uc_map_dump_section(self, file:str):           
        status = _uc.uc_map_dump_section(self._uch,ctypes.c_char_p(file.encode('utf-8')))
        if status != uc.UC_ERR_OK:
            raise UcError(status)
    ...
    def mem_map_ptr(self, address: int, size: int, perms: int, ptr: int):
        ...
        if status != uc.UC_ERR_OK:
            raise UcError(status)
    # map a range of memory from a raw host memory address
    def mem_map_ptr_guest(self, address: int, size: int, perms: int, ptr: int):
        regaddress = ctypes.c_uint64(address)  
        status = _uc.uc_mem_map_ptr_guest(self._uch, ctypes.byref(regaddress), size, perms, ptr)
        if status != uc.UC_ERR_OK:
            raise UcError(status)
        return  regaddress.value 
    ...
    def hook_add(self, htype: int, callback: UC_HOOK_CALLBACK_TYPE , user_data: Any=None, begin: int=1, end: int=0, arg1: int=0, arg2: int=0):
        ...
        if htype == uc.UC_HOOK_INSN:
            ...
            if htype in (uc.UC_HOOK_BLOCK, uc.UC_HOOK_CODE, uc.UC_HOOK_HARDWARE_EXECUTE, uc.UC_HOOK_HARDWARE_READ, uc.UC_HOOK_HARDWARE_WRITE, uc.UC_HOOK_HARDWARE_READWRITE):
