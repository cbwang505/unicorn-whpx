...
class QlGdbUtils:
    ...
    def dbg_hook_safe(self, ql: Qiling):
        ql.stop()
        self.last_bp = ql.arch.regs.arch_pc
        self.ql.log.info(f'{PROMPT} dbg_hook_safe ql stop at {self.last_bp:#x}')
    ...
    def bp_insert(self, addr: int, size: int):
        ...
        self.bpdic[addr]=self.ql.hook_address(self.dbg_hook_safe,addr )

        return True 

    def bp_insert_hw(self, addr: int, size: int,type: int):
        targets = set(addr + i for i in range(size or 1))
        ...
        self.bpdic[addr]=self.ql.hook_address_hw(self.dbg_hook_safe,addr,type )
    ...
    def bp_remove(self, addr: int, size: int) -> bool:
        ...
        self.ql.hook_del(self.bpdic[addr])
        self.bpdic.pop(addr)
        return True
